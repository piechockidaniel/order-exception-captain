...
| Works consistently as depicted | complete for the synthetic demo | Automated validation has covered the 49-case suite, `uv build` with all dashboard assets, and a fresh scheduled scan with SQLite-integrity-checked backup. On 2026-09-04, ResolveDesk release `bea6a60` was verified live on the VPS: the public dashboard, brand assets, and health endpoint return `200`, while anonymous incident access returns `401`. Desktop and mobile HTTPS layouts were visually rechecked after release. |
| Public repository with visible MIT/Apache license and README | complete | Anonymous GitHub lookup confirms the repository is public and visibly MIT-licensed with a README. Public `main` contains ResolveDesk runtime release `bea6a60` and its deployment record, verified on 2026-09-04. |
